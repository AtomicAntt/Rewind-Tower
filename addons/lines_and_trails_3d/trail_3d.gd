...
@export var time_limit: float = 0.25:
	...
	set(value):
		if time_limit == value:
			return
		time_limit = value
		_moving_time_limit = minf(_moving_time_limit, time_limit)
		if _auto_rebuild and Engine.is_editor_hint():
			tick(0)
...
func tick(p_delta: float) -> void:
	...
	if not points.is_empty() and not pos.is_equal_approx(points[0]):
		_moving_time_limit = minf(time_limit, _moving_time_limit + p_delta)
	else:
		_moving_time_limit = maxf(0, _moving_time_limit - p_delta)
	...
	if limit_mode == LimitMode.LENGTH:
		...
		var extra_time := total_time - _moving_time_limit
...
func _refresh_ticking() -> void:

	set_process(limit_mode == LimitMode.TIME and tick_mode == TickMode.PROCESS)
	set_physics_process(limit_mode == LimitMode.TIME and tick_mode == TickMode.PHYSICS_PROCESS)
