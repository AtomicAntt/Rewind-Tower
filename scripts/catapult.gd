...
func fire_projectile() -> void:
	# First, get our global enemy position if one exists, else it exits this function.
	var global_enemy_position: Vector3
	if not is_instance_valid(closest_enemy_area):
		return
	else:
		var enemy_target: Enemy = closest_enemy_area.get_parent()
		global_enemy_position = enemy_target.global_position
	
	# Then, lets calculate the time it would take for the projectile to hit the floor (Y AXIS)
	...
	# So, we get the horizontal displacement
